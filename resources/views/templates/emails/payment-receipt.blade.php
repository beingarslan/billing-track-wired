@php
    // Colour the invoice status badge shown in the summary. Keyed off the invoice's
    // status_text; defaults to green since a payment has just been recorded.
    $statusRaw = strtolower(trim((string) ($payment->invoice->status_text ?? '')));
    if (str_contains($statusRaw, 'partial')) {
        ...
        $badgeBg = '#E7F4EE'; $badgeFg = '#1F7A5C';          // paid / other -> green
    }
@endphp
<!DOCTYPE html>
...
<head>
  ...
  <title>Payment received · Invoice {!! $payment->invoice->number !!}</title>
  ...
  <style>
    ...
    * { -ms-text-size-adjust:100%; -webkit-text-size-adjust:100%; }
    ...
    [data-ogsc] .panel     { background:#14251F !important; }
  </style>
</head>
...
<body class="bg-canvas" style="margin:0; padding:0; background:#EEEFF3; font-family:-apple-system,BlinkMacSystemFont,'Segoe UI',Roboto,Helvetica,Arial,sans-serif;">
  ...
  <div style="display:none; max-height:0; overflow:hidden; mso-hide:all; font-size:1px; line-height:1px; color:#EEEFF3; opacity:0;">
    Payment received — {!! $payment->formatted_amount !!} applied to invoice {!! $payment->invoice->number !!}. Thank you!
    ...
    <tr>
      <td align="center" style="padding:32px 12px;">
        ...
        <table role="presentation" width="600" cellpadding="0" cellspacing="0" border="0" class="container bg-paper brd-card"
          ...
          <tr>
            <td class="px" style="padding:26px 40px 22px 40px; border-bottom:1px solid #E6E7EC;" align="center">
              <!-- Logo. Change `width` to taste — height is auto, so the aspect ratio is preserved.
                   The white chip keeps the logo legible in dark mode; delete the wrapping <span> if not needed. -->
              <span style="display:inline-block; background:#FFFFFF; border-radius:10px; padding:8px 14px; line-height:0;">
                <img src="https://coduko.com/image/FinalCoduko-05.png"
                     alt="{!! $payment->invoice->companyProfile?->company !!}"
                     width="150"
          ...
          <!-- Success check (signature). Rounds everywhere except Outlook, where it shows as a green tile. -->
          <tr>
            <td style="padding:32px 40px 0 40px;" align="center">
              <div style="width:54px; height:54px; line-height:54px; border-radius:50%; background:#1F7A5C; color:#FFFFFF; font-size:26px; text-align:center; mso-line-height-rule:exactly;">✓</div>
            </td>
          </tr>

          <!-- Hero -->
          <tr>
            <td class="px" style="padding:18px 40px 8px 40px;" align="center">
              <div style="font-size:12px; font-weight:700; letter-spacing:1.2px; text-transform:uppercase; color:#1F7A5C;">
          ...
          <tr>
            <td class="px t-soft" style="padding:16px 40px 4px 40px; font-size:15px; line-height:1.65; color:#4A4D60;">
              Dear <strong style="color:inherit;">{!! $payment->invoice->client?->name ?? 'there' !!}</strong>,<br><br>
              Thank you! We've received your payment of {!! $payment->formatted_amount !!} and applied it to invoice {!! $payment->invoice->number !!}. A summary is below for your records.
            </td>
          </tr>
                ...
                <tr>
                  <td align="center" style="border-radius:8px;">
                    ...
                    <v:roundrect xmlns:v="urn:schemas-microsoft-com:vml" xmlns:w="urn:schemas-microsoft-com:office:word"
                                 href="{!! $payment->invoice->public_url !!}"
                    ...
                    <![endif]-->
                    <!--[if !mso]><!-->
                    <a class="btn" href="{!! $payment->invoice->public_url !!}"
          ...
          <tr>
            <td class="px t-muted" align="center" style="padding:8px 40px 4px 40px; font-size:12px; line-height:1.6; color:#9498A8;">
              ...
              <a class="txt-link" href="{!! $payment->invoice->public_url !!}" style="color:#1F7A5C; word-break:break-all;">{!! $payment->invoice->public_url !!}</a>
            </td>
          </tr>
            ...
            <td class="px" style="padding:24px 40px 6px 40px;">
              ...
              <table role="presentation" width="100%" cellpadding="0" cellspacing="0" border="0" style="font-size:14px;">
                ...
                <tr>
                  ...
                  <td class="t-ink brd-line" style="padding:12px 0; color:#1B1D2A; font-weight:600; text-align:right; border-bottom:1px solid #EDEEF2;">{!! $payment->payment_type !!}</td>
                </tr>
                <tr>
                  <td class="t-muted brd-line" style="padding:12px 0; color:#6E7180; border-bottom:1px solid #EDEEF2;">Reference</td>
                  <td class="t-ink brd-line" style="padding:12px 0; color:#1B1D2A; font-weight:600; text-align:right; border-bottom:1px solid #EDEEF2;">{!! $payment->transaction_reference !!}</td>
                </tr>
                --}}
                <tr>
                  <td class="t-muted" style="padding:12px 0; color:#6E7180;">Status</td>
                  <td style="padding:12px 0; text-align:right;">
                    <span style="display:inline-block; padding:3px 9px; border-radius:20px; background:{!! $badgeBg !!}; color:{!! $badgeFg !!}; font-size:11px; font-weight:700; letter-spacing:0.4px; text-transform:uppercase;">{!! $payment->invoice->status_text !!}</span>
                  </td>
                </tr>
              </table>
            </td>
          ...
          <tr>
            <td class="px" style="padding:6px 40px 0 40px;">
              <table role="presentation" width="100%" cellpadding="0" cellspacing="0" border="0" style="font-size:14px;">
                <tr>
                  <td class="t-soft" style="padding:10px 0 6px 0; color:#4A4D60;">Invoice total</td>
                  <td class="t-ink" style="padding:10px 0 6px 0; color:#1B1D2A; font-weight:600; text-align:right;">{!! $payment->invoice->amount->formatted_total !!}</td>
                </tr>
                <tr>
                  <td class="t-soft" style="padding:6px 0 14px 0; color:#4A4D60;">Total paid</td>
                  <td class="t-ink" style="padding:6px 0 14px 0; color:#1B1D2A; font-weight:600; text-align:right;">{!! $payment->invoice->amount->formatted_paid !!}</td>
                </tr>
                <tr>
                  <td class="t-ink brd-line" style="padding:16px 0 0 0; color:#1B1D2A; font-size:16px; font-weight:700; border-top:2px solid #1B1D2A;">Balance remaining</td>
                  <td class="t-ink brd-line" style="padding:16px 0 0 0; color:#1B1D2A; font-size:18px; font-weight:700; text-align:right; border-top:2px solid #1B1D2A;">{!! $payment->invoice->amount->formatted_balance !!}</td>
                </tr>
              </table>
            </td>
          </tr>
          ...
          <tr>
            <td class="px t-muted" style="padding:30px 40px 20px 40px; font-size:13px; line-height:1.6; color:#8A8D9C; border-top:1px solid #EDEEF2;">
              Have a question about this payment or invoice? Just reply to this email and our team will be happy to help.
            </td>
          </tr>
          ...
          <tr>
            <td class="px t-soft" style="padding:0 40px 32px 40px; font-size:14px; line-height:1.6; color:#4A4D60;">
              Best regards,<br>
              <strong class="t-ink" style="color:#1B1D2A;">{!! $payment->invoice->user?->name !!}</strong>
            </td>
          </tr>
        ...
        <table role="presentation" width="600" cellpadding="0" cellspacing="0" border="0" class="container" style="width:600px; max-width:600px;">
          <tr>
            <td class="px t-muted" align="center" style="padding:24px 40px 8px 40px; font-size:12px; line-height:1.7; color:#9498A8;">
              <strong style="color:#7E8294;">{!! $payment->invoice->companyProfile?->company !!}</strong>
          ...
          <tr>
            <td class="t-muted" align="center" style="padding:0 40px 8px 40px; font-size:12px; color:#B4B7C4;">
              © {!! date('Y') !!} {!! $payment->invoice->companyProfile?->company !!}. All rights reserved.
            </td>
          </tr>
        </table>
